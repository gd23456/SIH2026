...
review → price → publish. The publish screen must end on a **QR code** and a
working storefront link. If it does not, stage 3 has nothing worth wrapping.
...
| App loads but every screen is canned data | can't reach the backend | §4 |
...
| White screen for seconds on launch | offline font fetch | Already fixed — fonts load non-blocking |
...
`capacitor.config.json` sets `android.allowMixedContent`. Both are deliberate,
